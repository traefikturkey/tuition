...
export class DockerClient {
  ...
  async getContainer(nameOrId: string): Promise<ContainerStatus | null> {
    ...
    let container = containers.find((c) => c.id === nameOrId || c.name === nameOrId || c.name === `/${nameOrId}`);

    // Try partial match
    if (!container) {
      container = containers.find((c) => c.id.startsWith(nameOrId) || c.name.includes(nameOrId));
    }
